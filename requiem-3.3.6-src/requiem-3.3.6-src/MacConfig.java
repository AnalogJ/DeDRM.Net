...
class MacConfig extends Config.BaseConfig {
  ...
  static {
    try {
      File f = File.createTempFile("libNative", ".dylib");
      Util.copy(MacConfig.class.getResourceAsStream("libNative" + System.getProperty("sun.arch.data.model") + ".dylib"), new FileOutputStream(f));
