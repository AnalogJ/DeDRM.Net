...
class UnDrmQueue {
  ...
  static void queueFile(File f) {
    ...
    try {
      Map<Long,Cipher> ciphers = UnDrm.getCiphers(f, notifier);
    ...
    synchronized (lock) {
      ...
      for (Task t : tasks) {
        ...
        try {
          ...
          if (g.exists()) throw new IOException("temporary file " + g + " in the way");
          UnDrm.unDrm(f, g, t.ciphers);
